...
export async function getLocation(url: string): Promise<[number, number, number]> {
    ...
    const regex = new RegExp(/\[(\d+[,.]\d{3}|\d+)[, ]+(\d+[,.]\d{3}|\d+)/)
...
//@todo Add a check for outliers and just spam this
export default async function owOCR(): Promise<[number, number, number]> {
    return await getLocation(await getScreenshotFromNewWorld().then(result => result)).then(result => result)
}